...
df_orders = pd.DataFrame(orders, columns=["OrderNo","Data","Empno"])
...
df_sales["Total"] = df_sales["Price"] * df_sales["Quantity"] # Criando a coluna Total e encontrando o valor total de cada venda.

df_sales = df_sales[["Data","Empno","Total"]] # Filtrando apenas as colunas que queremos.
#print(df_sales)

# df_emps_sales = df_sales.merge(df_emps)
# print(df_emps_sales)

df_sales_emps = df_sales.merge(df_emps)
df_result = df_sales_emps.merge(df_locations)
df_result = df_result[["Data", "Region", "Total"]]
