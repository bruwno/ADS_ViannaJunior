...
SELECT codigo_cliente FROM cliente WHERE telefone IS NOT NULL AND datanasc IS NULL;
...
SELECT codigo_imovel, descricao FROM imovel WHERE cidade = 'Juiz de Fora' AND bairro = 'Centro';

-- g) Selecione o código e a descrição dos imóveis situados no Rio de Janeiro ou Petrópolis.
SELECT codigo_imovel, descricao FROM imovel WHERE cidade = 'Rio de Janeiro' OR cidade = 'Petrópolis';

-- h) Selecione o nome e cpf do(s) proprietário(s) de nome João e sobrenome da Silva.
SELECT nome_proprietario, cpf FROM proprietario WHERE nome_proprietario = 'João' AND sobrenome = 'Silva';

-- i) Selecione as informações cujo tipo de imóvel é apartamento.
SELECT rua, bairro, cidade, cep, descricao FROM imovel WHERE codigo_tipo = 1;

-- j) Selecione todas as informações da tabela venda cujos valores dos imóveis sejam de R$150.000,00.
SELECT * FROM venda WHERE valor = 200000;
...
SELECT * FROM locacao WHERE valor_total > 2000.00;
...
SELECT * FROM locacao WHERE valor_total = 900.00;
